...
export class UserBusinessService {
  ...
  async batchPurchase(dto: BatchPurchaseDto) {
    return this.prisma.$transaction(async (tx) => {
      ...
      if (user.apple_balance < dto.total_cost) {
        throw this.createInsufficientBalanceError(user.apple_balance, dto.total_cost);
      }

      const userBusiness = await this.getOrCreateUserBusiness(tx, dto.business_id);
  ...
  private async validateBusiness(tx: Prisma.TransactionClient, businessId: number) {
  ...
  private async getOrCreateUserBusiness(tx: Prisma.TransactionClient, businessId: number) {
  ...
  private async updateUserBusinessLevel(tx: Prisma.TransactionClient, businessId: number, newLevel: number) {
  ...
  private async updateUserBalance(tx: Prisma.TransactionClient, newBalance: number) {
